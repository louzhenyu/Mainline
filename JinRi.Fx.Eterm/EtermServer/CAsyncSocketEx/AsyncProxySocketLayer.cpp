...
void CAsyncProxySocketLayer::SetProxy(int nProxyType, const TCHAR * pProxyHost, int ProxyPort)
{
	//Validate the parameters
...
void CAsyncProxySocketLayer::SetProxy(int nProxyType, const TCHAR * pProxyHost, int ProxyPort, const TCHAR * pProxyUser, const TCHAR * pProxyPass)
{
	//Validate the parameters
		...
		if (m_nProxyOpState==1) //Get respone to initialization message
		{
			...
			if (m_nRecvBufferPos==2)
			{
				...
				{ //Auth needed
					...
					//Send authentication
					LPCSTR lpszAsciiUser = CT2A(m_ProxyData.pProxyUser);
					LPCSTR lpszAsciiPass = CT2A(m_ProxyData.pProxyPass);
			...
			LPCSTR lpszAsciiHost = m_pProxyPeerHost?CT2A(m_pProxyPeerHost):"";
...
void CAsyncProxySocketLayer::OnConnect(int nErrorCode)
{
	...
	if (m_nProxyOpID==PROXYOP_CONNECT || m_nProxyOpID==PROXYOP_LISTEN)
	{
		...
		{ //SOCKS4 proxy
			//Send request
			LPCSTR lpszAscii = m_pProxyPeerHost ? CT2A(m_pProxyPeerHost) : "";
		...
		else if (m_ProxyData.nProxyType==PROXYTYPE_HTTP11)
		{
			...
			{
				pHost = new char[strlen(CT2A(m_pProxyPeerHost)) + 1];
				strcpy(pHost, CT2A(m_pProxyPeerHost));
			}
			else
			{
				pHost = new char[16];
			...
			else
			{
				...
				char userpass[4096];
				sprintf(userpass, "%s:%s", m_ProxyData.pProxyUser ? CT2A(m_ProxyData.pProxyUser) : "", m_ProxyData.pProxyPass ? CT2A(m_ProxyData.pProxyPass) : "");
				char base64str[4096];
