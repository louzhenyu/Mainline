...
BOOL CRegistry::Write(LPCTSTR lpValueName, LPCTSTR lpValue)
{
	...
	long lReturn = RegSetValueEx(m_hKey, lpValueName, 0L, REG_SZ, (const BYTE *)lpValue, 255);  //_tcslen(lpValue)+
...
BOOL CRegistry::Read(LPCTSTR lpValueName, CString* lpVal)
{
	...
	{
		lReturn=RegQueryValueEx(m_hKey,lpValueName,NULL,&dwType,(BYTE *)szString,&dwSize);
		if(lReturn==ERROR_SUCCESS)
		{
			*lpVal=szString;
			return TRUE;
