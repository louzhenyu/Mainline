...
﻿namespace ZooKeeperNet
{
    ...
    public class ZooKeeper : IDisposable, IZooKeeper
    {
        ...
        public long SessionId
        {
            get
            {
                //fixed by Yang Li: 当从客户端传来的connectstring是个非法格式的ip地址时，cncn为null：
        ...
        /// </summary>
        /// <param name="zk"></param>
        public static void WaitUntilConnected(ZooKeeper zk)
        {
            for (int i = 0; i < 5; i++)
            {
        ...
        public Stat Exists(string path, bool watch)
        {
            ...
            {
                //fixed by Yang Li, 解决有时第1次连ZooKeeper服务集群因异步连接而导致的ConnectionLossException异常
                return Exists(path, watch ? watchManager.DefaultWatcher : null);
            }
        }      
...
#warning code here
        ...
        //added by Yang Li at Feb.29th, 2016
        public byte[] GetRemark(string path, bool watch, Stat stat)
        {
            return GetRemark(path, watch ? watchManager.DefaultWatcher : null, stat);
        }
...
#warning 2
        ...
        //added by Li Yang at Feb.29th, 2016
        public Stat SetRemark(string path, byte[] remark, int version)
        {
            string clientPath = path;
            PathUtils.ValidatePath(clientPath);
            ...
            RequestHeader h = new RequestHeader();
#warning code here
            //h.Type = (int)OpCode.SetRemark;
            h.Type = (int)OpCode.SetData;
            SetRemarkRequest request = new SetRemarkRequest(serverPath, remark, version);
            SetRemarkResponse response = new SetRemarkResponse();
            ReplyHeader r = cnxn.SubmitRequest(h, request, response, null);
            if (r.Err != 0)
            {
                throw KeeperException.Create((KeeperException.Code)Enum.ToObject(typeof(KeeperException.Code), r.Err), clientPath);
