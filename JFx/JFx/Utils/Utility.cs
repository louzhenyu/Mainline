...
namespace JFx.Utils
{
    ...
    public class Utility
    {
        ...
        private static string safeExtension = ".XLS|.XLSX|.XML|.PDF|.JPEG|.JPG|.GIF|.PNG|.DOC|.DOCX|.TIFF|.PSD|.BMP";
        /// <summary>
        /// 验证上传文件是否安全
        /// <para>安全文件：.XLS|.XLSX|.XML|.PDF|.JPEG|.JPG|.GIF|.PNG|.DOC|.DOCX|.TIFF|.PSD|.BMP</para>
        /// </summary>
        /// <returns>是否成功</returns>
        public static bool PostFileValidation()
        {
            try
            {
                foreach (string key in HttpContext.Current.Request.Files.AllKeys)
                {
                    string str = System.IO.Path.GetExtension(HttpContext.Current.Request.Files[key].FileName);
                    if (!safeExtension.Contains(str.ToUpper()))
                    {
                        return false;
                    }
                }
