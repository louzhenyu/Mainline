...
namespace JinRi.Fx.Utility
{
    ...
    public class CommonHelper
    {
        ...
        public static string ConvertDateMethod(string columnName, IntervalUnit unit)
        {
            int length = 17;
            switch (unit)
            {
                case IntervalUnit.Minute: length = 17; break;
                case IntervalUnit.Hour: length = 14; break;
                case IntervalUnit.Day: length = 11; break;
            }
            return string.Format("CONVERT(NVARCHAR({0}), {1}, 21)", length, columnName);
        }
        /// <summary>
        /// 获取DateTime指定间隔的下一个DateTime
        /// </summary>
        /// <param name="currentTime">当前时间</param>
        /// <param name="timeInterval"></param>
        /// <param name="unit"></param>
        /// <returns></returns>
        public static DateTime GetNextTime(DateTime currentTime, int timeInterval, IntervalUnit unit)
        {
            DateTime returnValue = DateTime.Parse("1900-01-01");
            switch (unit)
            {
                case IntervalUnit.Minute: returnValue = currentTime.AddMinutes(timeInterval); break;
                case IntervalUnit.Hour: returnValue = currentTime.AddHours(timeInterval); break;
                case IntervalUnit.Day: returnValue = currentTime.AddDays(timeInterval); break;
            }
            return returnValue;
        }

        public static DateTime GetDateByUnit(IntervalUnit unit, DateTime dateTime)
        {
            switch (unit)
            {
                ...
                case IntervalUnit.Hour:
                    return Convert.ToDateTime(dateTime.ToString("yyyy-MM-dd HH") + ":00");
        ...
        private static string GetDateFormat(IntervalUnit unit)
        {
        ...
        /// <returns></returns>
        public static List<string> GetCronFireTime(string cronExpression, int numTimes)
        {
